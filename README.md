...
因为路由规则需要，需要开启rewrite而且把`web`目录设置为站点根目录。

apache的`.htaccess`文件我已经配置好放在`web`目录里了。vhost配置可以和以下那么简单（视情况而定，我在windows用的是集成环境PHPStudy，一般直接用集成环境自带的添加网站就好，当然懂自己动手是坠吼的）：
