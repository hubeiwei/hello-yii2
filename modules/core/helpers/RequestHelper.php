...
class RequestHelper
{
     ...
     * @param \mix|string $params 查询参数，形如bar=foo&foo=bar；或者是一维关联数组，形如array('a'=>'aa',...)
     * @param   string $method 请求方式，是POST还是GET
     * @param array|string $my_header 用户要发送的头部信息，为一维关联数组，形如array('a'=>'aa',...)
     * @return  array                       成功返回一维关联数组，形如array('header'=>'bar', 'body'=>'foo')，
     *                                      重大错误程序直接停止运行，否则返回false。
     */
    static function request($url, $params = '', $method = 'POST', $my_header = '')
    {
//        $fsock_exists = function_exists('fsockopen');
        ...
        if (self::$time_limit > -1) //如果为0，不限制执行时间
        {
...
//        if ($fsock_exists && !self::$use_curl) {
//            $response = self::use_socket($url, $params, $method, $my_header);
//        } /* 只要上述条件中的任一个不成立，流程就转向这里，这时如果curl模块可用，就调用use_curl函数 */
//        elseif ($curl_exists) {
            $response = self::use_curl($url, $params, $method, $my_header);
//        }
     ...
     *                                      否则返回false。
     */
//    static function use_socket($url, $params, $method, $my_header)
//    {
//        $query = '';
//        $auth = '';
//        $content_type = '';
...
//    }
     ...
     *                                      失败返回false。
     */
    static function use_curl($url, $params, $method, $my_header)
    {
        /* 开始一个新会话 */
        ...
        curl_setopt($curl_session, CURLOPT_SSL_VERIFYPEER, FALSE);//不验证证书
        curl_setopt($curl_session, CURLOPT_SSL_VERIFYHOST, FALSE);//不验证证书
        $url_parts = self::parse_raw_url($url);
        ...
        if (self::$connect_timeout > -1) {
            curl_setopt($curl_session, CURLOPT_CONNECTTIMEOUT, self::$connect_timeout);
        }

        if (self::$stream_timeout > -1) {
            curl_setopt($curl_session, CURLOPT_TIMEOUT, self::$stream_timeout);
        }
     ...
     * @return     array
     */
    static function parse_raw_url($raw_url)
    {
        $retval = array();
